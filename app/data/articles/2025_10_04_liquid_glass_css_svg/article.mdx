...
<blockquote className="mt-8 text-[15px] leading-7 border-l-2 pl-4 border-zinc-300/70 dark:border-white/15 text-zinc-800/90 dark:text-zinc-100/90">
  <span className="font-semibold tracking-[0.2em] uppercase">TL;DR</span>
  <span className="mx-2 text-zinc-400/80">—</span>
  <a
    href="#demo"
    className="font-medium underline decoration-dotted underline-offset-4 hover:decoration-2"
  >
    Jump to the demo →
  </a>
</blockquote>
...
The relationship between the incoming and outgoing light angles is described by [**Snell's Law**](https://en.wikipedia.org/wiki/Snell%27s_law):
...
- When n2 < n1, the ray bends away from the normal, and depending on the angle of incidence, it may bend so much that it reflects back into the original medium instead of passing through. This is called [Total Internal Reflection](https://en.wikipedia.org/wiki/Total_internal_reflection)
- When incident ray is orthogonal to the surface, it passes straight through regardless of refractive indices.

## What are Light Rays in this context?

In our simulation, we're modeling how light travels from your screen to your eyes. Each "ray" represents the path a photon takes as it bounces off screen pixels, passes through our virtual glass surface, and reaches your retina.

<ScreenLightToEyeDiagram />

There is one big thing to understand though. These rays are not the actual light rays going from your screen to your eyes. Because they are all orthogonal to the screen. They are just a simulation in the device, where there is There is no perspective. But this will allow us to simplify the math a lot, and we would not know anyway the actual position of your eyes.

This is a point that allows a big simplification of the math and code.
...
- Incident rays are always orthogonal to the background plane.
...
Displacing the ray outside of the object is a behavior we want to avoid, as it requires to access the background outside of the object. Apple seems to only use convex surfaces in Liquid Glass (with the exception of the Switch components, which we will cover later). It is important then to understand that the final displacement will be less performant if we try to simulate concave surfaces.

- The background arrow shows displacement: how far a ray lands vs. without glass. Its color encodes magnitude (longer → more purple) and is reused in the visualization below.

- Symmetry: rays at the same border distance have the same incidence and displacement magnitude. Compute once and reuse around the bezel or object.
...
This allows us to calculate everything in two dimensions once (x and z axis), on one "half-slice" of the object, and we will the rotate these pre-calculated vectors around the z-axis, which will move these vectors in the y-axis.

We do that for two reasons:

- Performance, even if this is not a big optimisation, given the fact that the main bottleneck here is the fact that everything will be done in the JS main thread.
- Simplicity, because we can focus on a single 2D, and not worry about the 3D geometry of the object. (the object is just a revolution of this 2D slice).
...
- Simulate rays at different distances from the border on a single 2D slice of the object.
- Flatten these rays into a 1D array of displacement magnitudes. (Projection)
- Reduce these displacement magnitudes to be a ratio of the maximum possible displacement. (Normalization)
  This allows us to see all arrows with no overlap, to get a better visualisation. We also keep the color of the arrows to help visualise the magnitude.
...
Now we need to translate our mathematical refraction calculations into something the browser can actually render. We'll use [**SVG displacement maps**](https://developer.mozilla.org/en-US/docs/Web/SVG/Reference/Element/feDisplacementMap)—a technique that shifts pixels based on color values in an image.

## What is a Displacement Map?

A **displacement map** is simply an image where each pixel's color tells the browser how to move the corresponding pixel in another image. Think of it like a field of arrows, where each arrow (encoded as colors) points in the direction a pixel should be moved.
...
# Adding the Finishing Touch: Specular Highlights
...
## What Creates Specular Highlights?

In the real world, specular highlights occur when light reflects directly off a smooth surface toward your eye.

This specular highlight simulates the way light reflects off curved surfaces, creating bright edges where the surface normal aligns with a specified light direction. The effect is most prominent along the bezel edges where the surface curves meet the viewing angle, giving objects that characteristic glassy, three-dimensional appearance.

The calculation involves taking the dot product between the surface normal at each point along the border and a directional vector (typically representing the light source direction). This dot product gives us a measure of how aligned these vectors are—when they're perfectly aligned, we get maximum specular intensity, and when they're perpendicular, we get no specular reflection.

In our implementation, we apply this specular calculation only to the bezel region, creating a subtle rim lighting effect that enhances the perception of depth and material properties of the glass surface.

<SpecularPreview />

# Compositing the Filter

To combine the refraction displacement map and the specular highlight, we can use SVG's `<feImage />` to overlay the specular image on top of the displacement map. This way, we can apply both effects in a single filter.
