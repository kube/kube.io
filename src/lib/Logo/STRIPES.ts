...
const STRIPES_COUNT = 3;
const MARGIN_RATIO = 0.11;
const INNER_MARGIN_RATIO = 0.27;

const WIDTH = 1 - MARGIN_RATIO;
const STRIPE_WIDTH = (WIDTH / STRIPES_COUNT) * (1 - INNER_MARGIN_RATIO);
const INNER_MARGIN = (WIDTH - STRIPE_WIDTH * STRIPES_COUNT) / (STRIPES_COUNT - 1);
...
export const STRIPES = [
	...
	.map(([angleX, angleY, angleZ]) => {
		...
		return FACE_STRIPES.map(face => face.map(_ => _.multiplyByMatrix(transformMatrix)));
	})
